    ...
    IEnumerator Moving() {
        switch (startMv) {
            case startMoving.key1:
                while (!GameManager.instance.isOpenKeyBox1) {
                    yield return new WaitForSeconds(0.5f);
                }
                break;
            case startMoving.door1:
                while (!GameManager.instance.isOpenDoor1) {
                    yield return new WaitForSeconds(0.5f);
                }
                break;
            case startMoving.key2:
                while (!GameManager.instance.isOpenKeyBox2) {
                    yield return new WaitForSeconds(0.5f);
                }
                break;
            default:
        ...
        isMoving = true;
        while (!GameManager.instance.isRotating && rayHitPlayerUp.collider == null) {
            // Move
